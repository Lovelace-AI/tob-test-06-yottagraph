...
- Lovelace Brand R2 theming (lovelaceDark Vuetify theme)
- Agent Chat (via Portal Gateway)
- Entity Lookup (Query Server integration)
- MCP Tool Explorer
...
- Interactive project initialization system
- Feature module documentation and templates
...
- 2026-03-13: Removed example modules (event-sender, event-receiver, module-debug). Entity Lookup promoted to core page. Home page redesigned as concierge/onboarding screen with Agent Chat, Entity Lookup, and MCP Explorer as primary actions.
- 2026-03-13: Removed Cesium globe feature module and all dependencies (CDN scripts, cesium-loader util, cesium cursor rule). App no longer supports location-based data.
- 2026-03-13: Added `lovelaceDark` Vuetify theme to nuxt.config.ts matching the Broadchurch Portal. Added Vuetify component defaults (rounded buttons, outlined cards, etc.). Updated `useNewsTheme.ts` to activate `lovelaceDark` instead of generic `dark`.
