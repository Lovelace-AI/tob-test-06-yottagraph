...
async function init() {
    ...
    // Step 6: Update files
    console.log('\n🔧 Step 6: Configuring your project...\n');
...
${
    includeExamples
        ? `- **Entity Lookup** - Search the Query Server for named entities`
...
`;
    ...
    if (!includeExamples) {
        ...
        const featurePath = path.join(process.cwd(), 'features', 'entity-lookup');
        if (fs.existsSync(featurePath)) {
            fs.rmSync(featurePath, { recursive: true, force: true });
            console.log('   Removed entity-lookup');
        }
        ...
        if (fs.existsSync(pluginPath)) {
            ...
            pluginContent = pluginContent.replace(
                /import\s+\w+\s+from\s+['"]~\/features\/entity-lookup['"]\s*;?\s*\n/g,
                ''
            );
    ...
    if (includeExamples) {
        console.log('3. Explore the built-in features:');
